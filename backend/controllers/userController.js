...
const sendOTP = asyncHandler(async (req, res) => {
  ...
  const subject = "OTP CODE - traderconnects";
  ...
  if (twofaAuthentication !== true && verifyEmailResendOtp !== true) {
    ...
    const subjectAdmin = "New User Registration - traderconnects";
    const send_to_Admin = process.env.EMAIL_USER;
    const templateAdmin = NewUserEmailTemplate("Admin", user);
    const reply_toAdmin = "no_reply@traderconnects.net";
...
const idVerificationUpload = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Check if the current ID URLs exist before attempting any deletion
    ...
    const subjectAdmin = "ID Verification Request - traderconnects";
    const send_to_Admin = process.env.EMAIL_USER;
...
const adminApproveId = asyncHandler(async (req, res) => {
  ...
  const subject = "ID Approval Status - traderconnects";
  const send_to = user.email;
  ...
  if (status === "VERIFIED") {
    ...
    const messages = [
      {
        to: user.email,
        from: "Support Team",
        subject: "Welcome to traderconnects",
        content: `Hello ${
          user.firstname + " " + user.lastname
        }, We're excited to have you on board. traderconnects is an international investment company that combines the infrastructure and abilities of an investor with a best-in-class team of operations professionals. This unique combination of skills  has allowed us to become a top international Investment Platform.For more enquiry kindly contact your account manager or write directly with our live chat support on our platform or you can send a direct mail to us at support@traderconnects.net.`,
      },
    ];
    ...
    const notificationObject = {
      ...
      message: `Your trade account has been activated successfully. Welcome to traderconnects`,
      route: "/dashboard",
    };
...
const adminSendCustomizedMail = asyncHandler(async (req, res) => {
  ...
  const reply_to = "no-reply@traderconnects.net";
...
const contactUs = asyncHandler(async (req, res) => {
  ...
  const subjectAdmin = "Contact Us - traderconnects";
  const send_to_Admin = process.env.EMAIL_USER;
  const templateAdmin = adminGeneralEmailTemplate("Admin", introMessage);
  const reply_toAdmin = "no_reply@traderconnects.net";
...
const requestCard = asyncHandler(async (req, res) => {
  ...
  const subjectAdmin = `${cardType} Request - traderconnects`;
  const send_to_Admin = process.env.EMAIL_USER;
  const templateAdmin = adminGeneralEmailTemplate("Admin", introMessage);
  const reply_toAdmin = "no_reply@traderconnects.net";
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const resetPasswordLink = `https://traderconnects.net/auth/reset-password/${resetToken}`;

    const subject = "Reset Password - traderconnects";
    const send_to = user.email;
...
const upgradeAccount = asyncHandler(async (req, res) => {
  ...
  const subjectAdmin = "Upgrade Account - traderconnects";
  const send_to_Admin = process.env.EMAIL_USER;
  const templateAdmin = adminGeneralEmailTemplate("Admin", introMessage);
  const reply_toAdmin = "no_reply@traderconnects.com";
